...
router.post('/register', (req, res) => {
  ...
  if (ownerCount === 0) {
    db.prepare(`INSERT INTO users (username, passwordHash, salt, role, owner, status, token, createdAt)
      VALUES (?, ?, ?, 'owner', '', 'active', '', ?)`)
      .run(username, hashPassword(password, salt), salt, new Date().toISOString());
...
router.post('/login', (req, res) => {
  ...
  db.prepare('UPDATE users SET token = ?, tokenExpire = ? WHERE username = ?').run(token, expire, username);
  return res.json({
    success: true,
    message: '登录成功',
...
router.post('/logout', (req, res) => {
  ...
  if (token) getDb().prepare("UPDATE users SET token = '' WHERE token = ?").run(token);
