...
router.get('/list', async (req, res) => {
  const list = await all('SELECT username, role, owner, status, createdAt FROM users WHERE owner = ?', req.user.username);
...
router.post('/', async (req, res) => {
  ...
  if (action === 'add') {
    ...
    await run(
      ...
      username, hashPassword(password, salt), salt, req.user.username, new Date().toISOString()
    );
  ...
  if (action === 'disable' || action === 'enable') {
    ...
    const t = await get('SELECT username, owner FROM users WHERE username = ?', target);
    ...
    await run('UPDATE users SET status = ? WHERE username = ?', newStatus, target);
  ...
  if (action === 'remove') {
    ...
    if (!t) return res.json({ success: false, message: '目标账号不存在' });
    if (t.owner !== req.user.username) return res.json({ success: false, message: '只能管理自己名下的账号' });
    await run('DELETE FROM users WHERE username = ?', target);
    await run('DELETE FROM profiles WHERE owner = ?', target);
    await run('DELETE FROM results WHERE owner = ?', target);
